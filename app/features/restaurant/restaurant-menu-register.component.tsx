...
export default function MenuRegister({ restaurantId, menusCount }: Props) {
	const [menus, setMenus] = useState<number[]>(
		Array.from({ length: 5 - menusCount > 0 ? 5 - menusCount : 1 }).map(
			(_, idx) => idx,
		),
	); // default value 포함 5개 이상 맞추기 (레이아웃때매 스크롤 탑 적용이 안됨)

	const menuAddButtonRef = useRef<HTMLDivElement>(null);
	...
	const handleSubmit = (id: number) => async (values: any) => {
		...
		// Next.js의 서버 액션이 호출되면 자동으로 업데이트된 데이터와 ui를 받기 때문에 클라이언트에서 아래와 같이 미리보기를 따로 제공 할 필요가 없음.
	...
	useEffect(() => {
		// 메뉴 추가시 메뉴 버튼으로 스크롤링
		menuAddButtonRef?.current?.scrollIntoView({
			behavior: 'smooth',
			block: 'end',
			inline: 'end',
		});
	...
	return (
		<div>
			...
			<div className="flex flex-col gap-8 " ref={menuAddButtonRef}>
