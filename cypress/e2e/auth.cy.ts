describe('로그인/회원가입 테스트', () => {
	...
	it('로그인 여부에 따라 매장 등록 페이지 접근 권한 설정', () => {
		// 미로그인 상태에서 /register 페이지 접근 시 /로 리다이렉트
		cy.visit('/register');
		cy.findByRole('heading', { name: 'logo' }).should('exist');
		...
		// 로그인 상태에서 /register 페이지 접근 시 register 페이지 렌더링
		cy.findByRole('button', { name: Cypress.env('USER_NAME') }).click();
		cy.findByText('매장등록').click();
		cy.findByRole('heading', { name: 'logo' }).should('not.exist');
